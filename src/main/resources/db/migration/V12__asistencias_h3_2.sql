-- ============================================================================
-- V12 :: H3.2 - Ampliar tabla asistencias para check-in
--        Agrega columnas visitante + índice único parcial anti-duplicado
-- ============================================================================
...
BEGIN
    ...
    LOOP
        ...
        -- Columna: visitante_nombre
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = v_schema AND table_name = 'asistencias'
              AND column_name = 'visitante_nombre'
        ) THEN
            EXECUTE format(
                'ALTER TABLE %I.asistencias ADD COLUMN visitante_nombre VARCHAR(150)',
                v_schema);
        END IF;

        -- Columna: visitante_telefono
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = v_schema AND table_name = 'asistencias'
              AND column_name = 'visitante_telefono'
        ) THEN
            EXECUTE format(
                'ALTER TABLE %I.asistencias ADD COLUMN visitante_telefono VARCHAR(20)',
                v_schema);
        END IF;

        -- Índice único parcial: un miembro no puede registrarse dos veces en el mismo evento
        EXECUTE format(
            'CREATE UNIQUE INDEX IF NOT EXISTS uq_%s_asistencias_evento_miembro
        ...
        EXECUTE format(
            'CREATE INDEX IF NOT EXISTS idx_%s_asistencias_evento
             ON %I.asistencias(evento_id)',
            replace(v_schema, '-', '_'), v_schema);

        RAISE NOTICE 'Schema %: asistencias ampliado OK', v_schema;
    END LOOP;
END $$;
