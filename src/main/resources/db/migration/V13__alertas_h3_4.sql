-- ============================================================================
-- V13 :: H3.4 - Alertas de ausencia y tareas de seguimiento
-- ============================================================================

-- ── Umbral configurable en shared.sedes ───────────────────────────────────
ALTER TABLE shared.sedes
    ADD COLUMN IF NOT EXISTS umbral_ausencia_semanas INT NOT NULL DEFAULT 3;

-- ── Crear tablas en schemas de sedes existentes ───────────────────────────
DO $$
DECLARE
    v_schema TEXT;
BEGIN
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            WHERE table_schema = v_schema AND table_name = 'alertas_ausencia'
        ) THEN
            EXECUTE format('
                CREATE TABLE %I.alertas_ausencia (
                    ...
                    estado          VARCHAR(20) NOT NULL DEFAULT ''PENDIENTE''
                                    ...
                                    CHECK (estado IN (''PENDIENTE'',''GESTIONADA'',''DESCARTADA'')),
                    notas           TEXT,
            ...
            EXECUTE format(
                'CREATE UNIQUE INDEX uq_%s_alerta_miembro_pendiente
                 ON %I.alertas_ausencia(miembro_id)
        ...
        IF NOT EXISTS (
            SELECT 1 FROM information_schema.tables
            WHERE table_schema = v_schema AND table_name = 'tareas_seguimiento'
        ) THEN
            EXECUTE format('
        ...
        RAISE NOTICE 'Schema %: alertas y tareas creadas OK', v_schema;
    END LOOP;
END $$;

-- ── Actualizar fn_crear_schema_sede ───────────────────────────────────────
