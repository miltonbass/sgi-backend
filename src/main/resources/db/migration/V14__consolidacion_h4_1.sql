-- ============================================================================
-- V14 :: H4.1 - Consolidación de visitantes y nuevos miembros
-- ============================================================================

-- ── Límite configurable de asignados por consolidador ────────────────────
ALTER TABLE shared.sedes
    ADD COLUMN IF NOT EXISTS max_asignados_consolidador INT NOT NULL DEFAULT 10;

-- ── Crear tareas_consolidacion en schemas de sedes existentes ─────────────
DO $$
DECLARE
    v_schema TEXT;
BEGIN
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            WHERE table_schema = v_schema AND table_name = 'tareas_consolidacion'
        ) THEN
            EXECUTE format('
                CREATE TABLE %I.tareas_consolidacion (
                    ...
                    tipo            VARCHAR(30) NOT NULL DEFAULT ''PRIMER_CONTACTO''
                                    CONSTRAINT chk_tc_tipo
                                    CHECK (tipo IN (''PRIMER_CONTACTO'',''SEGUIMIENTO'')),
                    estado          VARCHAR(20) NOT NULL DEFAULT ''PENDIENTE''
                                    CONSTRAINT chk_tc_estado
            ...
            RAISE NOTICE 'Schema %: tareas_consolidacion creada OK', v_schema;
        END IF;
    END LOOP;
END $$;

-- ── Actualizar fn_crear_schema_sede (agrega tareas_consolidacion) ─────────
...
BEGIN
    ...
    -- TAREAS DE SEGUIMIENTO (ausencias)
    EXECUTE format('
    ...
    -- TAREAS DE CONSOLIDACION (primer contacto y seguimiento de visitantes)
    EXECUTE format('
        CREATE TABLE %I.tareas_consolidacion (
            id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
            sede_id UUID NOT NULL DEFAULT %L::UUID,
