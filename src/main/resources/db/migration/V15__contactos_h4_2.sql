-- ============================================================================
-- V15 :: H4.2 - Registro de contactos y seguimiento de consolidación
-- ============================================================================

-- ── Crear contactos_consolidacion en schemas existentes ──────────────────────
DO $$
DECLARE
    v_schema TEXT;
BEGIN
    ...
    LOOP
        ...
        IF NOT EXISTS (
            ...
            WHERE table_schema = v_schema AND table_name = 'contactos_consolidacion'
        ) THEN
            EXECUTE format('
                CREATE TABLE %I.contactos_consolidacion (
                    id              UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
                    sede_id         UUID NOT NULL,
                    miembro_id      UUID NOT NULL REFERENCES %I.miembros(id) ON DELETE CASCADE,
                    consolidador_id UUID NOT NULL REFERENCES %I.miembros(id),
                    fecha           DATE NOT NULL DEFAULT CURRENT_DATE,
                    tipo            VARCHAR(20) NOT NULL
                                    CONSTRAINT chk_contacto_tipo
                                    CHECK (tipo IN (''LLAMADA'',''VISITA'',''MENSAJE'',''REUNION'')),
            ...
            EXECUTE format(
                'CREATE INDEX idx_%s_contactos_miembro
                 ON %I.contactos_consolidacion(miembro_id, fecha DESC)',
                replace(v_schema, '-', '_'), v_schema);

            EXECUTE format(
                'CREATE INDEX idx_%s_contactos_consolidador
                 ON %I.contactos_consolidacion(consolidador_id)',
                replace(v_schema, '-', '_'), v_schema);

            RAISE NOTICE 'Schema %: contactos_consolidacion creada OK', v_schema;
        END IF;
    END LOOP;
END $$;

-- ── Actualizar fn_crear_schema_sede (agrega contactos_consolidacion) ─────────
...
BEGIN
    ...
    EXECUTE format(
        'CREATE UNIQUE INDEX uq_%s_asistencias_evento_miembro
         ON %I.asistencias(evento_id, miembro_id) WHERE miembro_id IS NOT NULL',
        replace(p_schema_name, '-', '_'), p_schema_name);

    -- ALERTAS DE AUSENCIA
    EXECUTE format('
        CREATE TABLE %I.alertas_ausencia (
            id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
            sede_id UUID NOT NULL DEFAULT %L::UUID,
            miembro_id UUID NOT NULL REFERENCES %I.miembros(id) ON DELETE CASCADE,
            semanas_ausente INT NOT NULL,
            consolidador_id UUID REFERENCES %I.miembros(id) ON DELETE SET NULL,
            estado VARCHAR(20) NOT NULL DEFAULT ''PENDIENTE''
                CONSTRAINT chk_alerta_estado CHECK (estado IN (''PENDIENTE'',''GESTIONADA'',''DESCARTADA'')),
            notas TEXT,
            gestionada_por UUID REFERENCES shared.usuarios_sistema(id),
            gestionada_en TIMESTAMPTZ,
            created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
        )', p_schema_name, p_sede_id, p_schema_name, p_schema_name);
    EXECUTE format(
        'CREATE UNIQUE INDEX uq_%s_alerta_miembro_pendiente
         ON %I.alertas_ausencia(miembro_id) WHERE estado = ''PENDIENTE''',
        replace(p_schema_name, '-', '_'), p_schema_name);
    EXECUTE format(
        'CREATE INDEX idx_%s_alertas_estado
         ON %I.alertas_ausencia(estado, created_at DESC)',
        replace(p_schema_name, '-', '_'), p_schema_name);

    -- TAREAS DE SEGUIMIENTO
    EXECUTE format('
        CREATE TABLE %I.tareas_seguimiento (
            id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
            sede_id UUID NOT NULL DEFAULT %L::UUID,
            alerta_id UUID NOT NULL REFERENCES %I.alertas_ausencia(id) ON DELETE CASCADE,
            miembro_id UUID NOT NULL REFERENCES %I.miembros(id) ON DELETE CASCADE,
            asignado_a UUID REFERENCES %I.miembros(id) ON DELETE SET NULL,
            descripcion TEXT NOT NULL,
            estado VARCHAR(20) NOT NULL DEFAULT ''PENDIENTE''
                CONSTRAINT chk_tarea_estado CHECK (estado IN (''PENDIENTE'',''COMPLETADA'')),
            completada_en TIMESTAMPTZ,
            created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
        )', p_schema_name, p_sede_id, p_schema_name, p_schema_name, p_schema_name);
    EXECUTE format(
        'CREATE INDEX idx_%s_tareas_asignado
         ON %I.tareas_seguimiento(asignado_a, estado)',
        replace(p_schema_name, '-', '_'), p_schema_name);

    -- TAREAS DE CONSOLIDACION
    EXECUTE format('
        CREATE TABLE %I.tareas_consolidacion (
            id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
            sede_id UUID NOT NULL DEFAULT %L::UUID,
            miembro_id UUID NOT NULL REFERENCES %I.miembros(id) ON DELETE CASCADE,
            consolidador_id UUID REFERENCES %I.miembros(id) ON DELETE SET NULL,
            descripcion TEXT NOT NULL,
            tipo VARCHAR(30) NOT NULL DEFAULT ''PRIMER_CONTACTO''
                CONSTRAINT chk_tc_tipo CHECK (tipo IN (''PRIMER_CONTACTO'',''SEGUIMIENTO'')),
            estado VARCHAR(20) NOT NULL DEFAULT ''PENDIENTE''
                CONSTRAINT chk_tc_estado CHECK (estado IN (''PENDIENTE'',''COMPLETADA'')),
            notas TEXT,
            completada_en TIMESTAMPTZ,
            completada_por UUID REFERENCES shared.usuarios_sistema(id),
            created_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
        )', p_schema_name, p_sede_id, p_schema_name, p_schema_name);
    EXECUTE format(
        'CREATE INDEX idx_%s_tc_consolidador
         ON %I.tareas_consolidacion(consolidador_id, estado)',
        replace(p_schema_name, '-', '_'), p_schema_name);
    EXECUTE format(
        'CREATE INDEX idx_%s_tc_miembro
         ON %I.tareas_consolidacion(miembro_id)',
        replace(p_schema_name, '-', '_'), p_schema_name);

    -- CONTACTOS DE CONSOLIDACION
    EXECUTE format('
        CREATE TABLE %I.contactos_consolidacion (
            id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
            sede_id UUID NOT NULL DEFAULT %L::UUID,
            ...
            fecha DATE NOT NULL DEFAULT CURRENT_DATE,
            tipo VARCHAR(20) NOT NULL
                CONSTRAINT chk_contacto_tipo CHECK (tipo IN (''LLAMADA'',''VISITA'',''MENSAJE'',''REUNION'')),
            resumen TEXT NOT NULL,
            proxima_accion TEXT,
            fecha_proxima_accion DATE,
            recordatorio_activo BOOLEAN NOT NULL DEFAULT FALSE,
