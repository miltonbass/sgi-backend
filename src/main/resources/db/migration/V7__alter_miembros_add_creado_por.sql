-- src/main/resources/db/migration/V7__alter_miembros_add_creado_por.sql
-- ============================================================
-- SGI - Historia 2.1 - Agregar creado_por a miembros en
--       todos los schemas de sede existentes, y actualizar
--       fn_crear_schema_sede para nuevas sedes.
-- ============================================================

-- 1. Actualizar el template (fn_crear_schema_sede) para nuevas sedes
...
-- 2. Alterar schemas de sedes YA EXISTENTES
--    Itera sobre todas las sedes activas y agrega creado_por si no existe
DO $$
DECLARE
    v_schema TEXT;
BEGIN
    ...
    LOOP
        ...
        -- Cambiar default de estado a VISITOR si sigue siendo ACTIVO
        EXECUTE format(
            'ALTER TABLE %I.miembros
             ALTER COLUMN estado SET DEFAULT ''VISITOR''',
            v_schema
        );

        -- Agregar índice en email si no existe
        EXECUTE format(
            'CREATE INDEX IF NOT EXISTS idx_%s_miembros_email
             ON %I.miembros(email) WHERE deleted_at IS NULL',
            replace(v_schema, '-', '_'), v_schema
        );

        RAISE NOTICE 'Schema % actualizado', v_schema;
    END LOOP;
END $$;